...
<img src="images/class_diagram_7.png" alt="">

### Интерфейс "IQuatable"

#### Определяет обобщенный метод для определения равенства экземпляров класса
...
| ExistingCoordinate | Статическая функция, определяющая корректность координаты                                                              |
...
| IsAbleToKill   | Данное свойство указывает на способность "рубить" в текущем ходу                                                                                                                              |
| Game           | Данное свойство хранит экземпляр текущей игры                                                                                                                                                 |
...
| AvailableMoves           | Данный метод возвращает доступные дла данной шашки клетки для хода                                                                       |
| AvailableMovesForLady    | Данный метод возвращает доступные клетки для хода для дамки                                                                              |
| AvailableMovesForRegular | Данный метод возвращает доступные клетки для хода для обычной шашки                                                                      |
...
| IsContinuingMove  | Данный метод позволяет определить, продолжает ли игрок свой ход                                                                  |
...
| Checkers        | Данное свойство хранит все экземпляры шашек игры                               |
...
| Checkers      | Данное свойство хранит все 24 шашки (12 белых и 12 черных)                                       |
...
| AppendPlayer           | Данный метод позволяет добавить экземпляр игрока в массив Players                                                                   |
| AppendChecker          | Данный метод позволяет добавить экземпляр шашки в массив Checkers                                                                   |
...
| OppositePlayer         | Данный метод позволяет получить противоположного игрока (который сейчас не ходит)                                                   |
| GetCheckerByCoordinate | Данный метод позволяет получить из массива Checkers шашку по координатам                                                            |